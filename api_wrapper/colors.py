...
RoboInit()
def checkHeadChest(): 
	global y1,y2,HeadY,chestY,index
	if (blocks[index].signature == 5):
		y1=blocks[index].y
	if (blocks[index].y != y1):
		y2=blocks[index].y
	if (y1 > y2):
		chestY=y2
		HeadY=y1
		...
		chestY=y1
		chestY=y2
	print 'index=%d HeadY=%d  chestY=%d  y1=%d  y2=%d'%(index, HeadY,chestY,y1,y2)
...
#	 print 'count' , count
	 ...
	 Uframe=frame/500
         ...
         for index in range(0, count):  #count how many blocks within the frame
		# pixy reselution 320X200	     
             	print '[index =%d Block_type=%d  Sig=%d  X=%3d  Y=%3d   W=%3d   H=%3d]' % (index, blocks[index].type, blocks[index].signature, blocks[index].x, blocks[index].y, blocks[index].width, blocks[index].height)
